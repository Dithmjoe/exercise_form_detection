...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        frame2= cv2.resize(frame, (640, 480))  # Change size as needed
        cv2.putText(frame2, f"Push-ups: {pushup_count}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
        cv2.putText(frame2, f"Leg State: {leg_state}", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        cv2.putText(frame2, f"Push-ups2: {mycount}", (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
        cv2.putText(frame2, f"angleleft: {left_leg_angle}", (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
        cv2.putText(frame2, f"angleleft: {right_leg_angle}", (50, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
        cv2.putText(frame2, f"elbowleft: {left_angle}", (50, 300), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
        cv2.putText(frame2, f"elbowRight: {right_angle}", (50, 350), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)

        # Render the pose landmarks on the frame (optional for visualization)
        mp.solutions.drawing_utils.draw_landmarks(frame2, results.pose_landmarks, mp_pose.POSE_CONNECTIONS)

    # Show the live frame with push-up count
    cv2.imshow("Push-up and Leg Detection", frame2)
